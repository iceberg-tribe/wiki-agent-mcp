...
[**🛠️ Tools**](tools.md) | [**🧠 Storage**](storage.md) | [**🧪 Testing**](testing.md) | [**📜 ADRs**](adr/001_premium_session_architecture.md)
...
│   ├── session.py       # Disk-backed caching of full articles and outlines
│   └── persistent.py    # SQLite-backed storage for summaries and analytics
├── domain/              # Business Logic: core types and reporting services
│   ├── models.py        # Dataclasses and shared types
│   └── reporting.py     # Proactive guidance and research dossiers
...
4. **Memory & Cache Update**: Full results are saved to the `wiki_cache/` folder via `storage/session.py`, while analytical summaries are stored in the `wiki_memory.db` via `storage/persistent.py`.
5. **Guidance & Reporting**: The `ReportingService` in `domain/` analyzes historical gaps and provides proactive "Next Step" suggestions or final Dossiers.
